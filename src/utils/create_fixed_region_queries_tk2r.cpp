...
void print_help(char * argv0) {
    printf("Usage: %s <n_queries> <k2_raster_file> <query_filename> <row-size> <col-size> <time-size> <value-size>\n",
           argv0);
}
...
void run_create_queries(std::string inputs_filename, std::string query_filename, size_t n_queries,
        size_t row_size, size_t col_size, size_t time_size, size_t value_size) {
    ...
    k2raster::create_fixed_region_queries<size_t, int>(n_queries, query_file,
            0, n_rows -1, row_size, 0, n_cols -1, col_size, 0, tend-1, time_size, valini, valend, value_size);
}
...
int main(int argc, char **argv) {
    ...
    size_t row_size = atol(argv[4]);
    size_t col_size = atol(argv[5]);
    size_t time_size = atol(argv[6]);
    size_t value_size = atol(argv[7]);
    ...
    switch (k2_raster_type) {
        case k2raster::K2_RASTER_TEMPORAL_TYPE:
            run_create_queries<k2raster::k2_raster_temporal<>>(k2raster_filename, query_filename, n_queries,
                    row_size, col_size, time_size, value_size);
            break;
        case k2raster::K2_RASTER_TEMPORAL_TYPE_H:
            run_create_queries<k2raster::k2rth_type>(k2raster_filename, query_filename, n_queries,
                                                     row_size, col_size, time_size, value_size);
            break;
