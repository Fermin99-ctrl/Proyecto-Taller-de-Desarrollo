...
void run_info(std::string k2raster_filename) {
    ...
    /* Print Info       */
    /*********************/
    int min_value = k2_raster.get_min_value();
    int max_value = k2_raster.get_max_value();
    std::cout << "k2-raster Min: " << min_value << " || Max: " << max_value << std::endl;
}
...
int main(int argc, char **argv) {
    ...
    switch (k2_raster_type) {
        case k2raster::K2_RASTER_TEMPORAL_TYPE:
            run_info<k2raster::k2_raster_temporal<>>(k2raster_filename);
            break;
        case k2raster::K2_RASTER_TEMPORAL_TYPE_H:
            run_info<k2raster::k2rth_type>(k2raster_filename);
            break;
        case k2raster::T_K2_RASTER_TYPE:
            run_info<k2raster::t_k2_raster<>>(k2raster_filename);
            break;
        case k2raster::AT_K2_RASTER_TYPE:
            run_info<k2raster::atk2r_type>(k2raster_filename);
            break;
        case k2raster::K2_RASTER_TEMPORAL_GLOBAL_TYPE:
            run_info<k2raster::k2_raster_temporal_global<>>(k2raster_filename);
            break;
        default:
            print_help(argv[0]);
